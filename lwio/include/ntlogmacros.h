...
#define LOG_LEAVE_IF_STATUS_EE(status, EE) \
    do { \
        if (EE || status) \
        { \
            LWIO_LOG_DEBUG("LEAVE_IF: -> 0x%08x (%s) (EE = %d)", status, LwNtStatusToSymbolicName(status), EE); \
        } \
    } while (0)
...
#define LOG_LEAVE_IF_STATUS_EE_EX(status, EE, Format, ...) \
    do { \
        if (EE || status) \
        { \
            LWIO_LOG_DEBUG("LEAVE_IF: " Format " -> 0x%08x (%s) (EE = %d)", ## __VA_ARGS__, status, LwNtStatusToSymbolicName(status), EE); \
        } \
    } while (0)
