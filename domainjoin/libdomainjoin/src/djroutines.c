...
CENTERROR QueryInformation(PDOMAINJOININFO * ppDomainJoinInfo)
{
	...
	ceError =
	    CTAllocateMemory(sizeof(DOMAINJOININFO),
			     (PVOID *) & pDomainJoinInfo);
	BAIL_ON_CENTERIS_ERROR(ceError);
	...
	if (IsNullOrEmptyString(pDomainJoinInfo->pszDomainName)) {
		...
		ceError =
		    DJGetConfiguredWorkgroup(&pDomainJoinInfo->
					     pszWorkgroupName);
		BAIL_ON_CENTERIS_ERROR(ceError);
...
void FreeDomainJoinInfo(PDOMAINJOININFO pDomainJoinInfo)
{
	if (pDomainJoinInfo) {
