...
static
void CTFreeNVPair(PNVPAIR pNVPair)
{
	if (pNVPair->pszName)
...
static
void CTFreeNVPairList(PNVPAIR pNVPairList)
{
...
static
void CTFreeSection(PCFGSECTION pSection)
{
	if (pSection->pszName)
...
static PNVPAIR ReverseNVPairList(PNVPAIR pNVPairList)
{
	PNVPAIR pP = NULL;
...
static PCFGSECTION ReverseSectionsAndNVPairs(PCFGSECTION pSectionList)
{
	PCFGSECTION pP = NULL;
...
void CTFreeConfigSectionList(PCFGSECTION pSectionList)
{
...
static CENTERROR ParseHeader(FILE * fp, PDWORD pdwSignature)
{
...
CENTERROR
CTReadNextLine(FILE * fp, PSTR pszBuf, DWORD dwMaxLen, PBOOLEAN pbEndOfFile)
{
	CENTERROR ceError = CENTERROR_SUCCESS;

	*pbEndOfFile = 0;

	if (fgets(pszBuf, dwMaxLen, fp) == NULL) {
		if (feof(fp)) {
			*pbEndOfFile = 1;
		} else {
			ceError = CTMapSystemError(errno);
...
static
 CENTERROR
ReadNextDoubleByteLine(FILE * fp,
		       PSTR pszBuf, DWORD dwMaxLen, PBOOLEAN pbEndOfFile)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	DBLBYTE dbBuf;
	PSTR pszTmp = pszBuf;
...
CENTERROR
CTParseConfigFile(PCSTR pszFilePath,
		  ...
		  BOOLEAN bWindowsDoubleByteFormat)
{
	...
	while (!bEOF) {
		...
		if (bWindowsDoubleByteFormat) {
			...
			ceError =
			    ReadNextDoubleByteLine(fp, szBuf, 1024, &bEOF);
			BAIL_ON_CENTERIS_ERROR(ceError);

		} else {

			ceError = CTReadNextLine(fp, szBuf, 1024, &bEOF);
			BAIL_ON_CENTERIS_ERROR(ceError);
		...
		if (szBuf[0] == '[' && szBuf[dwLen - 1] == ']') {
			...
			ceError =
			    CTAllocateMemory(sizeof(CFGSECTION),
					     (PVOID *) & pSection);
			BAIL_ON_CENTERIS_ERROR(ceError);
			...
			ceError =
			    CTAllocateString(szBuf + 1, &pSection->pszName);
			BAIL_ON_CENTERIS_ERROR(ceError);
			...
			ceError =
			    CTAllocateMemory(pszTmp - szBuf + 1,
					     (PVOID *) & pszName);
			BAIL_ON_CENTERIS_ERROR(ceError);
			...
			ceError =
			    CTAllocateMemory(sizeof(NVPAIR),
					     (PVOID *) & pNVPair);
			BAIL_ON_CENTERIS_ERROR(ceError);
			...
			pNVPair->pszName = pszName;
			pszName = NULL;
			pNVPair->pszValue = pszValue;
			pszValue = NULL;
...
CENTERROR CTSaveConfigSectionListToFile(FILE * fp, PCFGSECTION pSectionList)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PNVPAIR pNVPair = NULL;
	...
	while (pSectionList) {
		...
		while (pNVPair) {
			...
			if (fprintf(fp, "    %s = %s\n",
				    pNVPair->pszName,
				    (IsNullOrEmptyString(pNVPair->pszValue) ? ""
				     : pNVPair->pszValue)) < 0) {
				ceError = CTMapSystemError(errno);
				BAIL_ON_CENTERIS_ERROR(ceError);
			}
...
CENTERROR
CTSaveConfigSectionList(PCSTR pszConfigFilePath, PCFGSECTION pSectionList)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	/*PNVPAIR pNVPair = NULL; */
	FILE *fp = NULL;
	...
	ceError =
	    CTAllocateMemory(strlen(pszConfigFilePath) + strlen(".gpagent") + 1,
			     (PVOID *) & pszTmpPath);
	BAIL_ON_CENTERIS_ERROR(ceError);
...
CENTERROR
CTCreateConfigSection(PCFGSECTION * ppSectionList,
		      PCFGSECTION * pCreatedSection, PCSTR pszSectionName)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PCFGSECTION pSectionList = NULL;
	PCFGSECTION pSection = NULL;
	...
	if (!pSection) {
		ceError =
		    CTAllocateMemory(sizeof(CFGSECTION), (PVOID *) & pSection);
		BAIL_ON_CENTERIS_ERROR(ceError);
...
CENTERROR
CTGetConfigValueBySectionName(PCFGSECTION pSectionList,
			      PCSTR pszSectionName,
			      PCSTR pszName, PSTR * ppszValue)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PCFGSECTION pSection = NULL;
	...
	ceError = CTGetConfigValueBySection(pSection, pszName, ppszValue);
...
CENTERROR
CTSetConfigValueBySectionName(PCFGSECTION pSectionList,
			      PCSTR pszSectionName,
			      PCSTR pszName, PCSTR pszValue)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PCFGSECTION pSection = NULL;
	PNVPAIR pNVPair = NULL;
	...
	if (IsNullOrEmptyString(pszSectionName) || IsNullOrEmptyString(pszName)) {
		ceError = CENTERROR_INVALID_PARAMETER;
		BAIL_ON_CENTERIS_ERROR(ceError);
	}
	...
	ceError = CTSetConfigValueBySection(pSection, pszName, pszValue);
	BAIL_ON_CENTERIS_ERROR(ceError);
...
CENTERROR
CTDeleteConfigSection(PCFGSECTION * ppSectionList, PCSTR pszSectionName)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PCFGSECTION pSection = NULL;
...
CENTERROR CTDeleteNameValuePairBySection(PCFGSECTION pSection, PCSTR pszName)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PNVPAIR pNVPair = NULL;
...
CENTERROR
CTDeleteNameValuePairBySectionName(PCFGSECTION pSectionList,
				   PCSTR pszSectionName, PCSTR pszName)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PCFGSECTION pSection = NULL;
...
CENTERROR
CTGetConfigValueBySection(PCFGSECTION pSection, PCSTR pszName, PSTR * ppszValue)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PNVPAIR pNVPair = NULL;
...
CENTERROR
CTSetConfigValueBySection(PCFGSECTION pSection, PCSTR pszName, PCSTR pszValue)
{
	CENTERROR ceError = CENTERROR_SUCCESS;
	PNVPAIR pNVPair = NULL;
	PNVPAIR pTmpNVPair = NULL;
	...
	if (pNVPair == NULL) {
		...
		if (!IsNullOrEmptyString(pszValue)) {
			ceError =
			    CTAllocateString(pszValue, &pNVPair->pszValue);
			BAIL_ON_CENTERIS_ERROR(ceError);
		}
