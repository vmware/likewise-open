...
unsigned int *create_uint_list(char **strlist)
{
    ...
    uint_list = (unsigned int*) malloc(sizeof(unsigned int) * list_len);
    if (uint_list == NULL) return NULL;

    memset((void*)uint_list, 0, sizeof(unsigned int) * list_len);

    /* copy mbs strings to wchar16_t strings */
    for (i = 0; strlist[i] && i < list_len; i++) {
        uint_list[i] = (unsigned int) atoi(strlist[i]);
    }
                           ...
                           void *val, const void *def)
{
    ...
    case pt_uint32:
        ...
        *valuint = (unsigned int)((value) ? atol(value) : *defuint);
        break;
    case pt_uint32_list:
        valuint_list = (unsigned int**)val;
        defstr = (char**)def;
        strlist = (value) ? get_value_list(value) : get_value_list(*defstr);
        *valuint_list = create_uint_list(strlist);
