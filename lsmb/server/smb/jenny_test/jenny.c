...
int parse( int argc, char *argv[], state *s)
{
  ...
  if (strlen(feature_name) != MAX_FEATURES) {
    printf("feature_name length is wrong, %u\n",
			(uint32_t) strlen(feature_name));
		return FALSE;
  }
  ...
  if (temp > MAX_DIMENSIONS) {
    printf("jenny: maximum number of dimensions is %d.  %u is too many.\n",
	   MAX_DIMENSIONS, temp);
    return FALSE;
  }
  s->ndim = (ub2)temp;
  ...
  for (i=1, j=0; i<argc; ++i) {
	...
	printf("jenny: dimensions must be smaller than %d.  %u is too big.\n",
	       MAX_FEATURES, temp);
	return FALSE;
      }
      if (temp < 2) {
