...
DWORD
SMBAccessTokenSetSchannel(
    HANDLE hAccessToken
    )
{
    DWORD dwError = 0;
    PSMB_API_HANDLE pAPIHandle = (PSMB_API_HANDLE) hAccessToken;

    BAIL_ON_INVALID_HANDLE(hAccessToken);
    ...
    pAPIHandle->variant.securityToken.bIsSchannel = TRUE;
