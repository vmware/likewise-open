			    ...
			    fstring out)
{
	...
	/* HACK ALERT! Remove spaces from home directory path */

	all_string_sub( templ, lp_winbind_replacement_char(), "_", 0 );
				    ...
				    uint32 group_rid)
{
	...
	s->username = talloc_strdup(s->state->mem_ctx, username);
        ...
        mapped_name = ws_name_replace( s->state->mem_ctx, s->domain,
				       s->username, *lp_winbind_replacement_char() );
	if ( mapped_name != s->username ) {
...
void winbindd_getpwuid(struct winbindd_cli_state *state)
{
	...
	winbindd_uid2sid_async(state->mem_ctx, state->request.data.uid, 
			       getpwuid_recv, WINBINDD_GETPWUID, state);
}
...
void winbindd_list_users(struct winbindd_cli_state *state)
{
	...
	/* Make listing users privileged operation until it is turned fully async. */
	if (!state->privileged) {
		DEBUG(3, ("winbindd_list_users: non-privileged access is denied\n"));
		goto done;
	}
