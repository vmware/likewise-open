...
char *ws_name_replace( TALLOC_CTX *mem_ctx, 
		       struct winbindd_domain *domain, 
		       char *name, char replace )
{
	...
	status = resolve_username_to_alias(mem_ctx, domain, 
					   name, &normalized );
	if ( NT_STATUS_IS_OK(status) ) {
		return normalized;
	}
	...
	if ( NT_STATUS_EQUAL( status, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND ) ) {
		DEBUG(5,("ws_name_replace: Setting domain %s offline\n",
			 domain->name));		
	...
	all_string_sub( normalized, " ", replace_char, 0 );
...
char* ws_name_return( TALLOC_CTX *mem_ctx, 
		      char *name, char replace )
{
	char replace_char[2] = { 0x0, 0x0 };
	char *normalized = NULL;
	...
	status = resolve_alias_to_username( mem_ctx, domain,
					    name, &normalized );
	if ( NT_STATUS_IS_OK(status) ) {		
		return normalized;
	}
