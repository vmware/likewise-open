...
#if defined(AIX) || defined(SUNOS5)
static bool winbindd_have_working_mlock()
{
	static bool checked_mlock = False;
	/* By default, we assume mlock works. */
	static bool have_working_mlock = True;

	if (!checked_mlock) {
		const size_t size = 8;
		void* address = SMB_MEMALIGN_ARRAY(unsigned char,
						   getpagesize(),
						   size);
		if (address) {
			if (mlock(address, size) == 0) {
				have_working_mlock = True;
				checked_mlock = True;
#if defined(HAVE_MUNLOCK)
				if (munlock(address, size) < 0) {
					DEBUG(0,("failed to munlock in mlock check: %s (%d)\n",
	...
	return have_working_mlock;
}
#else
static inline bool winbindd_have_working_mlock()
{
	return True;
}
#endif
				   ...
				   const char *pass)
{
	...
	return NT_STATUS_OK;
#else
	if (!winbindd_have_working_mlock()) {
		return NT_STATUS_OK;
	}
