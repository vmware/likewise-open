					   ...
					   struct winbindd_cli_state *state)
{
	DOM_SID sid;
	NTSTATUS result;
	bool skip_local_users = False;

	switch (state->request.original_cmd) {
	case WINBINDD_GETPWUID:
		skip_local_users = True;
		break;
	default:
		/* everyone else is ok for now */
		break;
	}
	...
	result = idmap_uid_to_sid(&sid, state->request.data.uid, skip_local_users);
