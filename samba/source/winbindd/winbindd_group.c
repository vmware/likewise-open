		       ...
		       char *gr_name, gid_t unix_gid)
{
	...
	mapped_name = ws_name_replace(mem_ctx, domain, gr_name, 
				      *lp_winbind_replacement_char());
	/* See if we converted to an alias */
	if ( mapped_name != gr_name ) {
				     ...
				     size_t *gr_mem_len)
{
	...
	if (state) {
		...
		if (sys_getpeereid(state->sock, &ret_uid)==0) {
			...
			status = idmap_uid_to_sid(&querying_user_sid, ret_uid, True);
			   ...
			   size_t *num_gr_mem, char **gr_mem, size_t *gr_mem_len)
{
	...
	char replace_char = *lp_winbind_replacement_char();	
	...
	*num_gr_mem = 0;
	...
	for ( i=0; i<max_depth && glist; i++ ) {
		...
		for (j=0; j<n_members; j++) {
			...
			p = strchr(members[j], *lp_winbind_separator());
			if ( p )
				p++;
			else
				p = members[j];
			
			mapped_name = ws_name_replace(members, domain, 
						      p, replace_char);
			if (mapped_name == p)
...
void winbindd_getgrnam(struct winbindd_cli_state *state)
{
	...
	tmp = ws_name_return( state->mem_ctx, 
			      state->request.data.groupname, 
			      *lp_winbind_replacement_char() );
...
#if 0
	tmp = ws_name_replace( state->mem_ctx, domain, name_group, 
			       *lp_winbind_replacement_char() );
	if ( tmp != name_group )
#endif
				     ...
				     enum lsa_SidType name_type )
{
	...
	mapped_name = ws_name_return( s->state->mem_ctx, raw_name, 
				      *lp_winbind_replacement_char());
...
void winbindd_list_groups(struct winbindd_cli_state *state)
{
	...
	/* Make listing groups privileged operation until it is turned fully async. */
	if (!state->privileged) {
		DEBUG(3, ("winbindd_list_groups: non-privileged access is denied\n"));
		request_error(state);
		return;
	}
...
void winbindd_getgroups(struct winbindd_cli_state *state)
{
	...
	real_name = ws_name_return( state->mem_ctx, 
			       state->request.data.username,
			       *lp_winbind_replacement_char() );
	...
	if (!parse_domain_user_talloc(state->mem_ctx, real_name,
		...
		s->username = talloc_strdup( state->mem_ctx, real_name );		
	}
