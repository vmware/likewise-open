...
static NTSTATUS be_init(struct idmap_domain *dom)
{
	...
	for (i=0; i<num_domains; i++) {
		...
		DEBUG(5,("lwopen:be_init() Adding %s (%s) -> %d\n",
...
static NTSTATUS nss_lwopen_init(struct nss_domain_entry *e )
{
	return be_init(NULL);
}
...
static NTSTATUS nss_lwopen_get_info(struct nss_domain_entry *e,
				    const DOM_SID *sid,
				    TALLOC_CTX *ctx,
				    ADS_STRUCT *ads,
				    LDAPMessage *msg,
...
static NTSTATUS nss_lwopen_map_to_alias(TALLOC_CTX *mem_ctx,
					const char *domain,
					const char *name,
					char **alias)
{
...
static NTSTATUS nss_lwopen_map_from_alias(TALLOC_CTX *mem_ctx,
					  const char *domain,
					  const char *alias,
					  char **name)
{
...
static NTSTATUS nss_lwopen_close(void)
{
	return NT_STATUS_OK;
}
...
static struct idmap_methods lwopen_idmap_methods = {
	.init            = be_init,
...
static struct nss_info_methods lwopen_nss_methods = {
	.init           = nss_lwopen_init,
	.get_nss_info   = nss_lwopen_get_info,
	.map_to_alias   = nss_lwopen_map_to_alias,
	.map_from_alias = nss_lwopen_map_from_alias,
	.close_fn       = nss_lwopen_close
};
...
NTSTATUS idmap_lwopen_init(void)
{
	...
	if ( !NT_STATUS_IS_OK(idmap_status) ) {
		idmap_status =  smb_register_idmap(SMB_IDMAP_INTERFACE_VERSION,
						   "lwopen", &lwopen_idmap_methods);

		if ( !NT_STATUS_IS_OK(idmap_status) ) {
			DEBUG(0,("Failed to register lwopen idmap plugin.\n"));
			return idmap_status;
		}
	}
	...
	if ( !NT_STATUS_IS_OK(nss_status) ) {
		nss_status = smb_register_idmap_nss(SMB_NSS_INFO_INTERFACE_VERSION,
						    "lwopen", &lwopen_nss_methods);
		if ( !NT_STATUS_IS_OK(nss_status) ) {
			DEBUG(0,("Failed to register lwopen idmap nss plugin.\n"));
			return nss_status;
		}
	}
