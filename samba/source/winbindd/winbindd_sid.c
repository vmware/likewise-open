...
void winbindd_lookupname(struct winbindd_cli_state *state)
{
	...
	DEBUG(3, ("[%5lu]: lookupname %s%s%s\n", (unsigned long)state->pid,
		  name_domain, 
		  name_domain[0] ? lp_winbind_separator() : "",
		  name_user));
...
static void sid_to_principal_lookupsid_recv(void *private_data, bool success,
					    const char *dom_name,
					    const char *name,
					    enum lsa_SidType type)
{
	...
	if (!domain) {
		DEBUG(5, ("failed to find domain '%s'\n", dom_name));
		request_error(state);
		return;
	}

	if (!domain->alt_name || !domain->alt_name[0] || !strchr(domain->alt_name, '.')) {
		DEBUG(5, ("bad realm '%s' for domain '%s'\n", domain->alt_name ? domain->alt_name : "(null)", dom_name));
		request_error(state);
		return;
	}

	/* Note that a Windows DC does not care about the case of the
	   (non-realm part of) the principal name.  We canonicalize
	   to lowercase.  Note that this may not be appropriate for
	   machine accounts or other keytab situations.  However, this is
	   all temporary until we can get the principal name properly by
	   looking it up AD. */

	strupper_m(domain->alt_name);

	/* There should really be some error checking for lengths here to
	   make sure we do not return a success response with truncated data */

	fstrcpy(state->response.data.name.name, name);
	strlower_m(state->response.data.name.name);
	fstrcat(state->response.data.name.name, "@");
	fstrcat(state->response.data.name.name, domain->alt_name);

	DEBUG(5, ("principal is '%s'\n", state->response.data.name.name));

	state->response.data.name.type = type;
	request_ok(state);
}

void winbindd_sid_to_principal(struct winbindd_cli_state *state)
{
	DOM_SID sid;
	...
	DEBUG(3, ("[%5lu]: sid_to_principal %s\n", (unsigned long)state->pid, 
		  state->request.data.sid));
	...
	winbindd_lookupsid_async(state->mem_ctx, &sid,
				 sid_to_principal_lookupsid_recv, state);
}
...
void winbindd_uid_to_sid(struct winbindd_cli_state *state)
{
	...
	winbindd_uid2sid_async(state->mem_ctx, state->request.data.uid, 
			       uid2sid_recv, WINBINDD_UID_TO_SID, state);
}
