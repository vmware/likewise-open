 ...
 * idmap_likewise
 *
 * Unified Cell Provider for RFC2307 and Non-Schema mode cells
 *
...
static bool check_forest_scope(const char *dn)
{
	...
	if ((p = strstr_m(dn, LW_CELL_RDN)) == NULL) {
				...
				enum id_type *type)
{
	...
	if (cell_flags(c) & LWCELL_FLAG_USE_RFC2307_ATTRS) {
		...
		if (is_object_class(oc_list, list_size, LW_OC_POSIX_USER)
		    || is_object_class(oc_list, list_size, AD_USER)) {
			*type = ID_TYPE_UID;
		} else if (is_object_class(oc_list, list_size, LW_OC_POSIX_GROUP)
		...
		if (strequal(s, LW_OC_USER)) {
			*type = ID_TYPE_UID;
		} else if (strequal(s, LW_OC_GROUP)) {
			*type = ID_TYPE_GID;
		} else {
			*type = ID_TYPE_NOT_SPECIFIED;
			nt_status = NT_STATUS_INVALID_PARAMETER;
			      ...
			      uint32_t *id)
{
	...
	switch (type) {
	case ID_TYPE_UID:
		id_attr = LW_ATTR_UIDNUM;
		break;
	case ID_TYPE_GID:
		id_attr = LW_ATTR_GIDNUM;
		break;
	default:
		nt_status = NT_STATUS_INVALID_PARAMETER;
		BAIL_ON_NTSTATUS_ERROR(nt_status);
			      ...
			      gid_t *p_gid)
{
	...
	nt_status = get_object_string(c, msg, ctx, LW_ATTR_HOMEDIR, homedir);
	BAIL_ON_NTSTATUS_ERROR(nt_status);

	nt_status = get_object_string(c, msg, ctx, LW_ATTR_SHELL, shell);
	BAIL_ON_NTSTATUS_ERROR(nt_status);

	nt_status = get_object_string(c, msg, ctx, LW_ATTR_GECOS, gecos);
	/* Gecos is often not set so ignore failures */

	nt_status = get_object_uint32(c, msg, LW_ATTR_GIDNUM, p_gid);
	BAIL_ON_NTSTATUS_ERROR(nt_status);
