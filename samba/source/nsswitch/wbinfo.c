...
static bool wbinfo_list_domains(bool list_all_domains, bool show_details)
{
	struct winbindd_request request;
	struct winbindd_response response;
...
int main(int argc, char **argv, char **envp)
{
	...
	struct poptOption long_options[] = {
		...
		{ "details", 0, POPT_ARG_NONE, 0, OPT_DETAILS, "Show additional details (when showing domain info)", NULL },
	...
	while((opt = poptGetNextOpt(pc)) != -1) {
		...
		case 't':
			...
			break;
		case 'm':
			if (!wbinfo_list_domains(false, show_details)) {
		...
		case OPT_LIST_ALL_DOMAINS:
			if (!wbinfo_list_domains(true, show_details)) {
				goto done;
			}
			break;
