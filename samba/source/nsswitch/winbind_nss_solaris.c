...
#ifdef NSS_DEBUG
#define NSS_DEBUG(str) syslog(LOG_DEBUG, "nss_centeris: %s", str);
#else
...
static NSS_STATUS _nss_lwidentity_setpwent_solwrap (nss_backend_t* be, void* args)
{
	NSS_DEBUG("_nss_lwidentity_setpwent_solwrap");
	return _nss_lwidentity_setpwent();
}

static NSS_STATUS
_nss_lwidentity_endpwent_solwrap (nss_backend_t * be, void *args)
{
	NSS_DEBUG("_nss_lwidentity_endpwent_solwrap");
	return _nss_lwidentity_endpwent();
}

static NSS_STATUS
_nss_lwidentity_getpwent_solwrap (nss_backend_t* be, void *args)
{
	NSS_STATUS ret;
	...
	ret = _nss_lwidentity_getpwent_r(result, buffer, 
				      buflen, errnop);

	if(ret == NSS_STATUS_SUCCESS)
		{
			snprintf(logmsg, 79, "_nss_lwidentity_getpwent_solwrap: Returning user: %s\n",
				 result->pw_name);
			NSS_DEBUG(logmsg);
			NSS_ARGS(args)->returnval = (void*) result;
		} else {
			snprintf(logmsg, 79, "_nss_lwidentity_getpwent_solwrap: Returning error: %d.\n",ret);
			NSS_DEBUG(logmsg);
		}
...
static NSS_STATUS
_nss_lwidentity_getpwnam_solwrap (nss_backend_t* be, void* args)
{
	NSS_STATUS ret;
	struct passwd* result = (struct passwd*) NSS_ARGS(args)->buf.result;

	NSS_DEBUG("_nss_lwidentity_getpwnam_solwrap");

	ret = _nss_lwidentity_getpwnam_r (NSS_ARGS(args)->key.name,
						result,
...
static NSS_STATUS
_nss_lwidentity_getpwuid_solwrap(nss_backend_t* be, void* args)
{
	NSS_STATUS ret;
	struct passwd* result = (struct passwd*) NSS_ARGS(args)->buf.result;
  
	NSS_DEBUG("_nss_lwidentity_getpwuid_solwrap");
	ret = _nss_lwidentity_getpwuid_r (NSS_ARGS(args)->key.uid,
				       result,
				       NSS_ARGS(args)->buf.buffer,
				       NSS_ARGS(args)->buf.buflen,
				       &NSS_ARGS(args)->erange);
...
static NSS_STATUS _nss_lwidentity_passwd_destr (nss_backend_t * be, void *args)
{
	SAFE_FREE(be);
	NSS_DEBUG("_nss_lwidentity_passwd_destr");
	return NSS_STATUS_SUCCESS;
}

static nss_backend_op_t passwd_ops[] =
{
	_nss_lwidentity_passwd_destr,
	_nss_lwidentity_endpwent_solwrap,		/* NSS_DBOP_ENDENT */
	_nss_lwidentity_setpwent_solwrap,		/* NSS_DBOP_SETENT */
	_nss_lwidentity_getpwent_solwrap,		/* NSS_DBOP_GETENT */
	_nss_lwidentity_getpwnam_solwrap,		/* NSS_DBOP_PASSWD_BYNAME */
	_nss_lwidentity_getpwuid_solwrap		/* NSS_DBOP_PASSWD_BYUID */
};

nss_backend_t*
_nss_lwidentity_passwd_constr (const char* db_name,
			    const char* src_name,
			    const char* cfg_args)
{
	nss_backend_t *be;
	...
	NSS_DEBUG("Initialized nss_centeris passwd backend");
	return be;
}
...
static NSS_STATUS _nss_lwidentity_setgrent_solwrap (nss_backend_t* be, void* args)
{
	NSS_DEBUG("_nss_lwidentity_setgrent_solwrap");
	return _nss_lwidentity_setgrent();
}

static NSS_STATUS
_nss_lwidentity_endgrent_solwrap (nss_backend_t * be, void *args)
{
	NSS_DEBUG("_nss_lwidentity_endgrent_solwrap");
	return _nss_lwidentity_endgrent();
}

static NSS_STATUS
_nss_lwidentity_getgrent_solwrap(nss_backend_t* be, void* args)
{
	NSS_STATUS ret;
	char* buffer = NSS_ARGS(args)->buf.buffer;
	int buflen = NSS_ARGS(args)->buf.buflen;
	...
	ret = _nss_lwidentity_getgrent_r(result, buffer, 
				      buflen, errnop);

	if(ret == NSS_STATUS_SUCCESS)
		{
			snprintf(logmsg, 79, "_nss_lwidentity_getgrent_solwrap: Returning group: %s\n", result->gr_name);
			NSS_DEBUG(logmsg);
			NSS_ARGS(args)->returnval = (void*) result;
		} else {
			snprintf(logmsg, 79, "_nss_lwidentity_getgrent_solwrap: Returning error: %d.\n", ret);
			NSS_DEBUG(logmsg);
		}
...
static NSS_STATUS
_nss_lwidentity_getgrnam_solwrap(nss_backend_t* be, void* args)
{
	NSS_STATUS ret;
	struct group* result = (struct group*) NSS_ARGS(args)->buf.result;

	NSS_DEBUG("_nss_lwidentity_getgrnam_solwrap");
	ret = _nss_lwidentity_getgrnam_r(NSS_ARGS(args)->key.name,
				      result,
				      NSS_ARGS(args)->buf.buffer,
				      NSS_ARGS(args)->buf.buflen,
				      &NSS_ARGS(args)->erange);
...
static NSS_STATUS
_nss_lwidentity_getgrgid_solwrap(nss_backend_t* be, void* args)
{
	NSS_STATUS ret;
	struct group* result = (struct group*) NSS_ARGS(args)->buf.result;

	NSS_DEBUG("_nss_lwidentity_getgrgid_solwrap");
	ret = _nss_lwidentity_getgrgid_r (NSS_ARGS(args)->key.gid,
				       result,
				       NSS_ARGS(args)->buf.buffer,
				       NSS_ARGS(args)->buf.buflen,
				       &NSS_ARGS(args)->erange);
...
static NSS_STATUS
_nss_lwidentity_group_destr (nss_backend_t* be, void* args)
{
	SAFE_FREE(be);
	NSS_DEBUG("_nss_lwidentity_group_destr");
	return NSS_STATUS_SUCCESS;
}

static nss_backend_op_t group_ops[] = 
{
	_nss_lwidentity_group_destr,
	_nss_lwidentity_endgrent_solwrap,
	_nss_lwidentity_setgrent_solwrap,
	_nss_lwidentity_getgrent_solwrap,
	_nss_lwidentity_getgrnam_solwrap,
	_nss_lwidentity_getgrgid_solwrap,
	_nss_lwidentity_getgroupsbymember_solwrap
}; 

nss_backend_t*
_nss_lwidentity_group_constr (const char* db_name,
			   const char* src_name,
			   const char* cfg_args)
{
	nss_backend_t* be;
	...
	NSS_DEBUG("Initialized nss_centeris group backend");
	return be;
}
...
static NSS_STATUS
_nss_lwidentity_ipnodes_getbyname(nss_backend_t* be, void *args)
{
...
static NSS_STATUS
_nss_lwidentity_hosts_getbyname(nss_backend_t* be, void *args)
{
	nss_XbyY_args_t *argp = (nss_XbyY_args_t*) args;
	struct winbindd_response response;
	struct winbindd_request request;
...
static NSS_STATUS
_nss_lwidentity_hosts_getbyaddr(nss_backend_t* be, void *args)
{
	NSS_STATUS ret;
	struct winbindd_response response;
	struct winbindd_request request;
...
static NSS_STATUS
_nss_lwidentity_common_endent(nss_backend_t* be, void *args)
{
        return (NSS_STATUS_UNAVAIL);
}

static NSS_STATUS
_nss_lwidentity_common_setent(nss_backend_t* be, void *args)
{
        return (NSS_STATUS_UNAVAIL);
}

static NSS_STATUS
_nss_lwidentity_common_getent(nss_backend_t* be, void *args)
{
        return (NSS_STATUS_UNAVAIL);
}

static nss_backend_t*
_nss_lwidentity_common_constr (nss_backend_op_t ops[], int n_ops)
{
	nss_backend_t* be;
...
static NSS_STATUS
_nss_lwidentity_common_destr (nss_backend_t* be, void* args)
{
	SAFE_FREE(be);
	return NSS_STATUS_SUCCESS;
}
...
static nss_backend_op_t ipnodes_ops[] = {
	_nss_lwidentity_common_destr,
	_nss_lwidentity_common_endent,
	_nss_lwidentity_common_setent,
	_nss_lwidentity_common_getent,
	_nss_lwidentity_ipnodes_getbyname,
	_nss_lwidentity_hosts_getbyaddr,
};

nss_backend_t *
_nss_lwidentity_ipnodes_constr(dummy1, dummy2, dummy3)
        const char      *dummy1, *dummy2, *dummy3;
{
	return (_nss_lwidentity_common_constr(ipnodes_ops,
...
static nss_backend_op_t host_ops[] = {
	...
	_nss_lwidentity_common_setent,
	_nss_lwidentity_common_getent,
	_nss_lwidentity_hosts_getbyname,
	_nss_lwidentity_hosts_getbyaddr,
};

nss_backend_t *
_nss_lwidentity_hosts_constr(dummy1, dummy2, dummy3)
        const char      *dummy1, *dummy2, *dummy3;
{
	return (_nss_lwidentity_common_constr(host_ops,
