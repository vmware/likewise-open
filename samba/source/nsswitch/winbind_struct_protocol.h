...
#define WINBINDD_SOCKET_DIR  "/tmp/.lwidentity"  /* Name of PF_UNIX dir */
#endif
 ...
 */
#define WINBINDD_SOCKET_DIR_ENVVAR "LWIAUTHD_SOCKET_DIR"

#define WINBINDD_PRIV_SOCKET_SUBDIR "lwidentity_privileged" /* name of subdirectory of lp_lockdir() to hold the 'privileged' pipe */
#define WINBINDD_DOMAIN_ENV  "LWIAUTHD_DOMAIN" /* Environment variables */
#define WINBINDD_DONT_ENV    "_NO_LWIAUTHD"
#define WINBINDD_LOCATOR_KDC_ADDRESS "LWIAUTHD_LOCATOR_KDC_ADDRESS"

/* Update this when you change the interface.  */
/* Set ethe version + 1000 is avoid conflicts with the Samba 
 * WINBIND_INTERFACE_VERSION */

#define WINBIND_INTERFACE_VERSION 1020
...
#if defined(SUNOS5)
/* Force 8-byte alignment on Solaris.  (We do this for Solaris only
   since we are so close to release.  The issue here is that, while
   Solairs x64 binaries correctly align to 8-bytes for int64_t,
   Solaris x86 aligns this type to only 4-byte alignment.
   The same issue may or may not also affect 64-bit vs 32-bit SPARC. */
typedef int64_t SMB_TIME_T __attribute__((aligned(8)));
#else
...
enum winbindd_cmd {
	...
	WINBINDD_LOOKUPNAME,
	WINBINDD_LOOKUPRIDS,
	WINBINDD_SID_TO_PRINCIPAL,   /* Like LOOKUPSID, but returns principal name */
