...
static NSS_STATUS winbindd_get_response_internal(struct winbindd_response *response, bool allow_error_reponse)
{
	...
	if (allow_error_reponse) {
		if (response->result == WINBINDD_PENDING) {
			return NSS_STATUS_NOTFOUND;
		}
	} else {
...
{
	return winbindd_get_response_internal(response, false);
}

/* Handle simple types of requests */

static NSS_STATUS winbindd_request_response_internal(int req_type, 
			    struct winbindd_request *request,
			    struct winbindd_response *response,
			    bool need_priv,
			    bool allow_error_reponse
			    )
{
	...
	while ((status == NSS_STATUS_UNAVAIL) && (count < 10)) {
		status = winbindd_send_request(req_type, need_priv ? 1 : 0, request);
		if (status != NSS_STATUS_SUCCESS) 
			return(status);
		status = winbindd_get_response_internal(response, allow_error_reponse);
		count += 1;
	}
...
NSS_STATUS winbindd_request_error_response(int req_type, 
			    struct winbindd_request *request,
			    struct winbindd_response *response)
{
	return winbindd_request_response_internal(req_type, request, response,
						  false, true);
}
