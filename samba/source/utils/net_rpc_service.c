					...
					const char **argv )
{
	...
	if (argc != 0 ) {
		d_printf("Usage: %s rpc service list\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
						...
						const char **argv )
{
	...
	if (argc != 1 ) {
		d_printf("Usage: %s rpc service status <service>\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
...
{
	d_printf("%s rpc service list               View configured Win32 services\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc service start <service>    Start a service\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc service stop <service>     Stop a service\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc service pause <service>    Pause a service\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc service resume <service>   Resume a paused service\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc service status <service>   View the current status of a service\n",
		 NET_PROGRAM_NAME);
