				      ...
				      enum pdb_value_state))
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam set %s <user> <value>\n",
			  NET_PROGRAM_NAME,
			  field);
		return -1;
	}
				...
				uint16 flag)
{
	...
	if ((argc != 2) || (!strequal(argv[1], "yes") &&
			    !strequal(argv[1], "no"))) {
		d_fprintf(stderr, "usage: %s sam set %s <user> [yes|no]\n",
			  NET_PROGRAM_NAME,
			  field);
		return -1;
	}
...
static int net_sam_set_pwdmustchangenow(int argc, const char **argv)
{
	...
	if ((argc != 2) || (!strequal(argv[1], "yes") &&
			    !strequal(argv[1], "no"))) {
		d_fprintf(stderr, "usage: %s sam set pwdmustchangenow <user> [yes|no]\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_set_comment(int argc, const char **argv)
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam set comment <name> "
			  "<comment>\n", NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_set(int argc, const char **argv)
{
	...
	return net_run_function2(argc, argv, "sam set", func);
}
...
static int net_sam_policy_set(int argc, const char **argv)
{
	...
	char *endptr;
        ...
        if (argc != 2) {
                d_fprintf(stderr, "usage: %s sam policy set " 
...
static int net_sam_policy_show(int argc, const char **argv)
{
	const char *account_policy = NULL;
        ...
        if (argc != 1) {
                d_fprintf(stderr, "usage: %s sam policy show"
			  " \"<account policy>\" \n",
			  NET_PROGRAM_NAME);
                return -1;
        }
...
static int net_sam_policy(int argc, const char **argv)
{
        ...
        return net_run_function2(argc, argv, "sam policy", func);
}
...
static int net_sam_rights_grant(int argc, const char **argv)
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam rights grant <name> "
			  "<right>\n", NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_rights_revoke(int argc, const char **argv)
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam rights revoke <name> "
			  "<right>\n", NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_rights(int argc, const char **argv)
{
	struct functable2 func[] = {
        ...
        return net_run_function2(argc, argv, "sam rights", func);
}
...
static int net_sam_mapunixgroup(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam mapunixgroup <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_unmapunixgroup(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam unmapunixgroup <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_createlocalgroup(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %ssam createlocalgroup <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_deletelocalgroup(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam deletelocalgroup <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_createbuiltingroup(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam createbuiltingroup <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_addmem(int argc, const char **argv)
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam addmem <group> <member>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_delmem(int argc, const char **argv)
{
	...
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s sam delmem <group> <member>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_sam_listmem(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam listmem <group>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
			   ...
			   struct pdb_search *search, const char *what)
{
	...
	if ((argc > 1) ||
		...
		d_fprintf(stderr, "usage: %s sam list %s [verbose]\n",
			  NET_PROGRAM_NAME, what);
		return -1;
	}
...
static int net_sam_list(int argc, const char **argv)
{
	...
	return net_run_function2(argc, argv, "sam list", func);
}
...
static int net_sam_show(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s sam show <name>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
int net_sam(int argc, const char **argv)
{
	...
	return net_run_function2(argc, argv, "sam", func);
}
