...
static int net_usershare_add_usage(int argc, const char **argv)
{
	...
	d_printf(
		"%s usershare add [-l|--long] <sharename> <path> [<comment>] [<acl>] [<guest_ok=[y|n]>]\n"
...
{
	d_printf(
		"%s usershare info [-l|--long] [wildcard sharename]\n"\
...
{
	d_printf(
		"%s usershare list [-l|--long] [wildcard sharename]\n"\
...
{
	d_printf("%s usershare add <sharename> <path> [<comment>] [<acl>] [<guest_ok=[y|n]>] to "
				"add or change a user defined share.\n"
		"%s usershare delete <sharename> to delete a user defined share.\n"
		"%s usershare info [-l|--long] [wildcard sharename] to print info about a user defined share.\n"
		"%s usershare list [-l|--long] [wildcard sharename] to list user defined shares.\n"
		"%s usershare help\n"\
		"\nType \"%s usershare help <option>\" to get more information on that option\n\n",
...
static int net_usershare_delete(int argc, const char **argv)
{
	...
	if (!validate_net_name(sharename, INVALID_SHARENAME_CHARS, strlen(sharename))) {
		d_fprintf(stderr, "%s usershare delete: share name %s contains "
                        "invalid characters (any of %s)\n",
			NET_PROGRAM_NAME, sharename, INVALID_SHARENAME_CHARS);
		SAFE_FREE(sharename);
		return -1;
	}
	...
	if (unlink(us_path) != 0) {
		d_fprintf(stderr, "%s usershare delete: unable to remove usershare %s. "
			"Error was %s\n",
			NET_PROGRAM_NAME, us_path, strerror(errno));
		SAFE_FREE(sharename);
		return -1;
	}
	SAFE_FREE(sharename);
...
static int net_usershare_add(int argc, const char **argv)
{
	...
	if (!validate_net_name(sharename, INVALID_SHARENAME_CHARS, strlen(sharename))) {
		d_fprintf(stderr, "%s usershare add: share name %s contains "
                        "invalid characters (any of %s)\n",
	...
	if (sys_stat(us_path, &sbuf) != 0) {
		d_fprintf(stderr, "%s usershare add: cannot stat path %s to ensure "
	...
	if ((myeuid != 0) && lp_usershare_owner_only() && (myeuid != sbuf.st_uid)) {
		d_fprintf(stderr, "%s usershare add: cannot share path %s as "
	...
	for (i = 0; i < num_aces; i++) {
		...
		switch(pcolon[1]) {
			...
			case 'R':
				break;
			default:
				d_fprintf(stderr, "%s usershare add: malformed acl %s "
		...
		if (!string_to_sid(&sid, name)) {
			...
			if (!NT_STATUS_IS_OK(ntstatus)) {
				d_fprintf(stderr, "%s usershare add: cannot convert name \"%s\" to a SID. %s.",
					NET_PROGRAM_NAME, name, get_friendly_nt_error_msg(ntstatus) );
	...
	if (guest_ok && !lp_usershare_allow_guests()) {
		d_fprintf(stderr, "%s usershare add: guest_ok=y requested "
	...
	if (write(tmpfd, file_img, to_write) != to_write) {
		d_fprintf(stderr, "%s usershare add: failed to write %u bytes to file %s. Error was %s\n",
			NET_PROGRAM_NAME, (unsigned int)to_write, full_path_tmp, strerror(errno));
...
int net_usershare(int argc, const char **argv)
{
	...
	if (lp_usershare_max_shares() == 0) {
		d_fprintf(stderr, "%s usershare: usershares are currently disabled\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
	...
	if (!dp) {
		int err = errno;
		d_fprintf(stderr, "%s usershare: cannot open usershare directory %s. Error %s\n",
			NET_PROGRAM_NAME, lp_usershare_path(), strerror(err) );
		if (err == EACCES) {
