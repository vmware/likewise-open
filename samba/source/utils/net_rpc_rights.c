					...
					const char **argv )
{
	...
	/* backward comaptibility: if no keyword provided, treat the key
	   as an account name */
	if (argc > 1) {
		d_printf("Usage: %s rpc rights list [[accounts|privileges] [name|SID]]\n",
			 NET_PROGRAM_NAME);
		result = NT_STATUS_OK;
		goto done;
	}
					...
					const char **argv )
{
	...
	if (argc < 2 ) {
		d_printf("Usage: %s rpc rights grant <name|SID> <rights...>\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
					...
					const char **argv )
{
	...
	if (argc < 2 ) {
		d_printf("Usage: %s rpc rights revoke <name|SID> <rights...>\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
...
{
	d_printf("%s rpc rights list [{accounts|privileges} [name|SID]]   View available or assigned privileges\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc rights grant <name|SID> <right>                  Assign privilege[s]\n",
		 NET_PROGRAM_NAME);
	d_printf("%s rpc rights revoke <name|SID> <right>                 Revoke privilege[s]\n",
		 NET_PROGRAM_NAME);
	
	d_printf("\nBoth 'grant' and 'revoke' require a SID and a list of privilege names.\n");
	d_printf("For example\n");
	d_printf("\n  %s rpc rights grant 'VALE\\biddle' SePrintOperatorPrivilege SeDiskOperatorPrivilege\n",
		 NET_PROGRAM_NAME);
