...
{
	d_printf(
	 "\n%s rap session [misc. options] [targets]"\
	 "\n\tenumerates all active SMB/CIFS sessions on target server\n",
	 NET_PROGRAM_NAME);
	d_printf(
	 "\n%s rap session DELETE <client_name> [misc. options] [targets] \n"\
	 "\tor"\
	 "\n%s rap session CLOSE <client_name> [misc. options] [targets]"\
	 "\n\tDeletes (closes) a session from specified client to server\n",
	 NET_PROGRAM_NAME, NET_PROGRAM_NAME);
	d_printf(
	"\n%s rap session INFO <client_name>"\
...
{
	d_printf("%s rap server [misc. options] [target]\n\t"\
...
{
	d_printf("%s rap domain [misc. options] [target]\n\tlists the"\
...
{
	d_printf(
	 "%s rap printq [misc. options] [targets]\n"\
	 "\tor\n"\
	 "%s rap printq list [<queue_name>] [misc. options] [targets]\n"\
	 "\tlists the specified queue and jobs on the target server.\n"\
	 "\tIf the queue name is not specified, all queues are listed.\n\n",
	 NET_PROGRAM_NAME, NET_PROGRAM_NAME);
	d_printf(
	 "%s rap printq delete [<queue name>] [misc. options] [targets]\n"\
...
int net_rap_user(int argc, const char **argv)
{
	...
	if (ret != 0) {
		DEBUG(1, ("%s rap user returned: %d\n", NET_PROGRAM_NAME, ret));
	}
	return ret;
}
...
{
	d_printf(
	 "%s rap groupmember LIST <group> [misc. options] [targets]"\
	 "\n\t Enumerate users in a group\n"\
	 "\n%s rap groupmember DELETE <group> <user> [misc. options] "\
	 "[targets]\n\t Delete sepcified user from specified group\n"\
	 "\n%s rap groupmember ADD <group> <user> [misc. options] [targets]"\
...
{
	d_printf("%s rap validate <username> [password]\n"\
...
{
	d_printf("%s rap service [misc. options] [targets] \n"\
		 "\tlists all running service daemons on target server\n",
		 NET_PROGRAM_NAME);
	d_printf("\n%s rap service START <name> [service startup arguments]"\
		 " [misc. options] [targets]"\
		 "\n\tStart named service on remote server\n",
		 NET_PROGRAM_NAME);
	d_printf("\n%s rap service STOP <name> [misc. options] [targets]\n"\
...
{
	d_printf(
	 "%s rap password <user> <oldpwo> <newpw> [misc. options] [target]\n"\
...
{
	d_printf(
   "%s rap admin <remote command> [cmd args [env]] [misc. options] [targets]"\
...
{
	d_printf("  %s rap domain \tto list domains \n"\
		 "  %s rap file \t\tto list open files on a server \n"\
		 "  %s rap group \tto list user groups  \n"\
		 "  %s rap groupmember \tto list users in a group \n"\
		 "  %s rap password \tto change the password of a user\n"\
		 "  %s rap printq \tto list the print queues on a server\n"\
		 "  %s rap server \tto list servers in a domain\n"\
		 "  %s rap session \tto list clients with open sessions to a server\n"\
		 "  %s rap share \tto list shares exported by a server\n"\
		 "  %s rap user \t\tto list users\n"\
		 "  %s rap validate \tto check whether a user and the corresponding password are valid\n"\
		 "  %s rap help\n"\
