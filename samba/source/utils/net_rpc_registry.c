...
{
	if (argc < 4) {
		d_fprintf(stderr, "usage: %s rpc registry setvalue <key> "
			  "<valuename> <type> [<val>]+\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
{
	if (argc != 2) {
		d_fprintf(stderr, "usage: %s rpc registry deletevalue <key> "
			  "<valuename>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
{
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s rpc registry createkey <key>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
...
{
	if (argc != 1) {
		d_fprintf(stderr, "usage: %s rpc registry deletekey <key>\n",
			  NET_PROGRAM_NAME);
		return -1;
	}
						...
						const char **argv )
{
	...
	if (argc != 1 ) {
		d_printf("Usage:    %s rpc registry enumerate <path> [recurse]\n",
			 NET_PROGRAM_NAME);
		d_printf("Example:  %s rpc registry enumerate 'HKLM\\Software\\Samba'\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
					...
					const char **argv )
{
	...
	if (argc != 2 ) {
		d_printf("Usage:    %s rpc registry backup <path> <file> \n",
			NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
...
static int rpc_registry_dump( int argc, const char **argv )
{
	...
	if (argc != 1 ) {
		d_printf("Usage:    %s rpc registry dump <file> \n",
			 NET_PROGRAM_NAME);
		return 0;
	}
...
static int rpc_registry_copy( int argc, const char **argv )
{
	...
	if (argc != 2 ) {
		d_printf("Usage:    %s rpc registry copy <srcfile> <newfile>\n",
			 NET_PROGRAM_NAME);
		return 0;
	}
					    ...
					    const char **argv)
{
	...
	if (argc <1 || argc > 2) {
		d_printf("Usage:    %s rpc registry getsd <path> <secinfo>\n",
			 NET_PROGRAM_NAME);
		d_printf("Example:  %s rpc registry getsd 'HKLM\\Software\\Samba'\n",
			 NET_PROGRAM_NAME);
		return NT_STATUS_OK;
	}
...
int net_rpc_registry(int argc, const char **argv) 
{
	...
	return net_run_function2(argc, argv, "rpc registry", func);
}
