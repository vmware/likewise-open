...
{
	d_printf(
"  %s lookup [host] HOSTNAME[#<type>]\n\tgives IP for a hostname\n\n"
"  %s lookup ldap [domain]\n\tgives IP of domain's ldap server\n\n"
"  %s lookup kdc [realm]\n\tgives IP of realm's kerberos KDC\n\n"
"  %s lookup pdc [domain|realm]\n\tgives IP of realm's kerberos KDC\n\n"
"  %s lookup dc [domain]\n\tgives IP of domains Domain Controllers\n\n"
"  %s lookup master [domain|wg]\n\tgive IP of master browser\n\n"
"  %s lookup name [name]\n\tLookup name's sid and type\n\n"
"  %s lookup sid [sid]\n\tGive sid's name and type\n\n"
"  %s lookup dsgetdcname [name] [flags] [sitename]\n\n",
...
static int net_lookup_name(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_printf("usage: %s lookup name <name>\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_lookup_sid(int argc, const char **argv)
{
	...
	if (argc != 1) {
		d_printf("usage: %s lookup sid <sid>\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_lookup_dsgetdcname(int argc, const char **argv)
{
	...
	if (argc < 1 || argc > 3) {
		d_printf("usage: %s lookup dsgetdcname "
			 "<name> <flags> <sitename>\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
