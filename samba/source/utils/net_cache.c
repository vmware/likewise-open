...
static int net_cache_add(int argc, const char **argv)
{
	...
	if (argc < 3) {
		d_printf("\nUsage: %s cache add <key string> <data string> <timeout>\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_cache_search(int argc, const char **argv)
{
	...
	if (argc < 1) {
		d_printf("Usage: %s cache search <pattern>\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
{
	d_printf("  %s cache add \t add add new cache entry\n",
		 NET_PROGRAM_NAME);
	d_printf("  %s cache del \t delete existing cache entry by key\n",
		 NET_PROGRAM_NAME);
	d_printf("  %s cache flush \t delete all entries existing in the cache\n",
		 NET_PROGRAM_NAME);
	d_printf("  %s cache get \t get cache entry by key\n",
		 NET_PROGRAM_NAME);
	d_printf("  %s cache search \t search for entries in the cache, by given pattern\n",
		 NET_PROGRAM_NAME);
	d_printf("  %s cache list \t list all cache entries (just like search for \"*\")\n",
		 NET_PROGRAM_NAME);
	return -1;
}
