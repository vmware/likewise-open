...
static int net_groupmap_add(int argc, const char **argv)
{
	...
	if ( !unixgrp[0] ) {
		d_printf("Usage: %s groupmap add {rid=<int>|sid=<string>} unixgroup=<string> [type=<domain|local|builtin>] [ntgroup=<string>] [comment=<string>]\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_modify(int argc, const char **argv)
{
	...
	if ( !ntgroup[0] && !sid_string[0] ) {
		d_printf("Usage: %s groupmap modify {ntgroup=<string>|sid=<SID>} [comment=<string>] [unixgroup=<string>] [type=<domain|local>]\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_delete(int argc, const char **argv)
{
	...
	if ( !ntgroup[0] && !sid_string[0]) {
		d_printf("Usage: %s groupmap delete {ntgroup=<string>|sid=<SID>}\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_set(int argc, const char **argv)
{
	...
	if ((argc < 1) || (argc > 2)) {
		d_printf("Usage: %s groupmap set \"NT Group\" "
...
static int net_groupmap_addmem(int argc, const char **argv)
{
	...
	if ( (argc != 2) || 
		...
		d_printf("Usage: %s groupmap addmem alias-sid member-sid\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_delmem(int argc, const char **argv)
{
	...
	if ( (argc != 2) || 
	     !string_to_sid(&alias, argv[0]) ||
	     !string_to_sid(&member, argv[1]) ) {
		d_printf("Usage: %s groupmap delmem alias-sid member-sid\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_listmem(int argc, const char **argv)
{
	...
	if ( (argc != 1) || 
	     !string_to_sid(&alias, argv[0]) ) {
		d_printf("Usage: %s groupmap listmem alias-sid\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
static int net_groupmap_memberships(int argc, const char **argv)
{
	...
	if ( (argc != 1) || 
		...
		d_printf("Usage: %s groupmap memberof sid\n",
			 NET_PROGRAM_NAME);
		return -1;
	}
...
{
	d_printf("%s groupmap add"\
		"\n  Create a new group mapping\n",
		NET_PROGRAM_NAME);
	d_printf("%s groupmap modify"\
		"\n  Update a group mapping\n",
		NET_PROGRAM_NAME);
	d_printf("%s groupmap delete"\
		"\n  Remove a group mapping\n",
		NET_PROGRAM_NAME);
	d_printf("%s groupmap addmem"\
		 "\n  Add a foreign alias member\n",
		 NET_PROGRAM_NAME);
	d_printf("%s groupmap delmem"\
		 "\n  Delete a foreign alias member\n",
		 NET_PROGRAM_NAME);
	d_printf("%s groupmap listmem"\
		 "\n  List foreign group members\n",
		 NET_PROGRAM_NAME);
	d_printf("%s groupmap memberships"\
		 "\n  List foreign group memberships\n",
		 NET_PROGRAM_NAME);
	d_printf("%s groupmap list"\
		"\n  List current group map\n",
		NET_PROGRAM_NAME);
	d_printf("%s groupmap set"\
		"\n  Set group mapping\n",
		NET_PROGRAM_NAME);
	d_printf("%s groupmap cleanup"\
