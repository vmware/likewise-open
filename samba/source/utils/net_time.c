...
*/
time_t cli_servertime(const char *host, struct sockaddr_storage *pss, int *zone)
{
...
"%s time seconds\n\tdisplays the time on a server in seconds since the start of the epoch\n\n"\
"\n",
...
/* display the time on a remote box in seconds since the start of the epoch */
static int net_time_seconds(int argc, const char **argv)
{
	time_t t = nettime(NULL);
