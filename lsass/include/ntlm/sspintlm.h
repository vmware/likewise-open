...
typedef struct _NTLM_SEC_BUFFER
{
    ...
    USHORT usMaxLength;
    DWORD dwOffset;
} __attribute__((__packed__)) NTLM_SEC_BUFFER, *PNTLM_SEC_BUFFER;
...
typedef struct _WIN_VERSION_INFO
{
    ...
    SHORT sBuild;
    DWORD dwReserved;
} __attribute__((__packed__)) WIN_VERSION_INFO, *PWIN_VERSION_INFO;
...
} __attribute__((__packed__)) NTLM_SIGNATURE, *PNTLM_SIGNATURE;
