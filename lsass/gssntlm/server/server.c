 ...
 * Abstract:
 *
 *       Initialization routines for server portion of gssntlm 
 *
...
DWORD
NTLMGssInitializeServer(void)
{
    DWORD dwError;
    LSA_STRING workstation = {0};
    LSA_STRING config = NULL_LSASTRING;
    ...
    dwError = NTLMInitializeAuthProviders(
                &workstation,
                &workstation,
                &config);
    BAIL_ON_NTLM_ERROR(dwError);
