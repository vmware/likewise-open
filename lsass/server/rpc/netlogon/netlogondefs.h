 ...
 *
 *        Netlogon rpc server definitions
 *
 * Authors: Rafal Szczesniak (rafal@likewise.com)
 *          Adam Bernstein (abernstein@vmware.com)
 */

#ifndef _NETLOGONSRVDEFS_H_
#define _NETLOGONSRVDEFS_H_

#define LSA_RPC_DIR                              CACHEDIR "/rpc"

#define NETLOGON_RPC_CFG_DEFAULT_LPC_SOCKET_PATH     LSA_RPC_DIR "/lsass"
#define NETLOGON_RPC_CFG_DEFAULT_LOGIN_SHELL         "/bin/sh"
#define NETLOGON_RPC_CFG_DEFAULT_HOMEDIR_PREFIX      "/home"
#define NETLOGON_RPC_CFG_DEFAULT_HOMEDIR_TEMPLATE    "%H/%U"
...
#define GLOBAL_DATA_LOCK(locked)                         \
    do {                                                 \
        int ret = 0;                                     \
        ret = pthread_mutex_lock(&gNetlogonSrvDataMutex);    \
        if (ret) {                                       \
...
#define GLOBAL_DATA_UNLOCK(locked)                       \
    do {                                                 \
        int ret = 0;                                     \
        if (!locked) break;                              \
        ret = pthread_mutex_unlock(&gNetlogonSrvDataMutex);  \
