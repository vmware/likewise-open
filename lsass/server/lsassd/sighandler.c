    ...
    )
{
    ...
    sprintf(szClientPath, LSA_CLIENT_PATH_FORMAT, (long)getpid());

    dwError = LsaCheckSockExists(szClientPath, &bFileExists);
    BAIL_ON_LSA_ERROR(dwError);

    if (bFileExists) {
        dwError = LsaRemoveFile(szClientPath);
        BAIL_ON_LSA_ERROR(dwError);
    }
    ...
    dwError = LsaChangePermissions(unixaddr.sun_path, S_IRWXU);
    BAIL_ON_LSA_ERROR(dwError);

    memset(&unixaddr, 0, sizeof(unixaddr));
    unixaddr.sun_family = AF_UNIX;
    sprintf(unixaddr.sun_path, "%s/%s", pszCachePath, LSA_SERVER_FILENAME);
