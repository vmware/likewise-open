...
VmdirDbGetDnToken(
    ...
    PVMDIRDB_DN_TOKEN* ppDnToken,
    PDWORD           pdwLenUsed
    );

static
PVMDIRDB_DN_TOKEN
VmdirDbReverseTokenList(
    ...
    )
{
    ...
    do
    {
        PVMDIRDB_DN_TOKEN pToken = NULL;
        DWORD dwLenUsed;

        dwError = VmdirDbGetDnToken(
    ...
    pDN->pTokenList = VmdirDbReverseTokenList(pDN->pTokenList);
    ...
    )
{
    DWORD dwError = 0;
    wchar16_t wszDot[] = { '.', 0};
    PVMDIRDB_DN_TOKEN pToken = pDN->pTokenList;
    PVMDIRDB_DN_TOKEN pDCToken = NULL;
    PVMDIRDB_DN_TOKEN pParentDNToken = NULL;
    ...
    if (pToken->tokenType != VMDIRDB_DN_TOKEN_TYPE_DC)
    {
    ...
    while (pToken && (pToken->tokenType != VMDIRDB_DN_TOKEN_TYPE_DC))
    {
        pToken = pToken->pNext;
    }
    pDCToken = pToken;
    ...
    )
{
    ...
    {
        pToken->tokenType = VMDIRDB_DN_TOKEN_TYPE_CN;
    ...
    {
        pToken->tokenType = VMDIRDB_DN_TOKEN_TYPE_OU;
    ...
    {
        pToken->tokenType = VMDIRDB_DN_TOKEN_TYPE_DC;
...
{
    PVMDIRDB_DN_TOKEN pP = NULL;
    PVMDIRDB_DN_TOKEN pQ = pTokenList;
    PVMDIRDB_DN_TOKEN pR = NULL;
