    ...
    )
{
    ...
    LSA_ASSERT(LSA_TRUST_DIRECTION_TWO_WAY == trustDirection || LSA_TRUST_DIRECTION_SELF == trustDirection);
    ...
    for (index = 0; index < gcMembershipCount; index++)
    {
        ...
        if (!LsaHashExists(pGroupHash, pSid));
        {
            dwError = LsaHashSetValue(pGroupHash, pSid, pSid);
            BAIL_ON_LSA_ERROR(dwError);
        }
    }
