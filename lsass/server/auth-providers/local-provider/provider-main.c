    ...
    )
{
    ...
    if (strcasecmp(pszName, "password-lifespan") == 0)
    {
    ...
    else if (strcasecmp(pszName, "password-change-warning-time") == 0)
    {
        dwError = LsaParseDateString(
                        pszValue,
    ...
    if (dwError != 0) 
    {
        LSA_LOG_ERROR("Local provider: Failed to parse value of %s [%s]",
                pszName, pszValue);
        dwError = 0;
        goto done;
    }    

    if (dwPasswdChangeInterval != 0)
    {
        dwError = LsaProviderLocal_SetPasswdChangeInterval(dwPasswdChangeInterval);
    }
    else if (dwPasswdChangeWarningTime != 0)
    {
        dwError = LsaProviderLocal_SetPasswdChangeWarningTime(dwPasswdChangeWarningTime);
    }
    
    if (dwError != 0) 
    {
        LSA_LOG_ERROR("Local provider: Failed to assign value of %s [%s] to global variable",
                pszName, pszValue);
        dwError = 0;
        goto done;
    }
