    ...
    )
{
    ...
    if (dwError == LWREG_ERROR_NO_SUCH_KEY_OR_VALUE)
    {
        LSA_LOG_ERROR("LSA pstore plugin '%s' is missing its configuration registry key '%s'",
                pszName, pszKeyPath);
    ...
    if (dwError == LWREG_ERROR_NO_SUCH_KEY_OR_VALUE)
    {
        LSA_LOG_ERROR("LSA pstore plugin '%s' is missing the '%s' configuration value from its configuration registry key '%s'",
    ...
    )
{
    ...
    {
        LSA_LOG_ERROR("LSA pstore plugin %s is missing a dispatch table",
                         PluginInfo->Name);
        dwError = ERROR_DLL_INIT_FAILED;
        GOTO_CLEANUP_EE(EE);
    }
    ...
    {
        LSA_LOG_ERROR("LSA pstore plugin %s is missing the Cleanup function",
                         PluginInfo->Name);
        dwError = ERROR_DLL_INIT_FAILED;
        GOTO_CLEANUP_EE(EE);
    }

    LSA_LOG_VERBOSE("Loaded LSA pstore plugin %s from %s",
    ...
    )
{
    ...
    for (i = 0; i < pluginCount; i++)
    {
        ...
        if (dwError)
        {
            LSA_LOG_ERROR("Failed to load plugin %s with error = %u (%s)",
        ...
        if (dwError)
        {
            if (method)
            {
                LSA_LOG_ERROR(
                        ...
                        LSA_SAFE_LOG_STRING(Operation), pluginNames[i],
                        method, dwError,
                        LSA_SAFE_LOG_STRING(LwWin32ExtErrorToName(dwError)));
            }
            else
