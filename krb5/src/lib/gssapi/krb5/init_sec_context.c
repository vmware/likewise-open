   ...
   int default_mech)
{
   ...
   ctx->gss_flags = (GSS_C_TRANS_FLAG | 
                     ((req_flags) & (GSS_C_MUTUAL_FLAG | GSS_C_REPLAY_FLAG |
                                     GSS_C_INTEG_FLAG | GSS_C_CONF_FLAG | GSS_C_DCE_STYLE |
                                     GSS_C_SEQUENCE_FLAG | GSS_C_DELEG_FLAG)));
   /* dce style implies mutal */
   if (ctx->gss_flags & GSS_C_DCE_STYLE) {
   ...
   krb5_context context)
{
   ...
   if (!(code = g_verify_token_header(ctx->mech_used,
				      &(ap_rep.length),
				      &ptr, KG_TOK_CTX_AP_REP,
				      input_token->length, 1))) {
	 /* the AP_REQ was wrapped in a token header */
   } else if ((code == G_WRONG_TOKID)
	      &&!(code = g_verify_token_header((gss_OID) ctx->mech_used,
					       &(ap_rep.length),
   ...
   if (ctx->gss_flags & GSS_C_DCE_STYLE) {
      krb5_int32 con_flags,flags;

      krb5_auth_con_getflags(context, ctx->auth_context, &con_flags);
      flags = con_flags;
      flags &= ~(KRB5_AUTH_CONTEXT_DO_SEQUENCE | KRB5_AUTH_CONTEXT_RET_SEQUENCE | KRB5_AUTH_CONTEXT_USE_SUBKEY);
      krb5_auth_con_setflags(context, ctx->auth_context, flags);

      if ((code = krb5_mk_rep(context, ctx->auth_context, &mk_rep))) {
         goto fail;
      }
   ...
   output_token->value = mk_rep.data;
   output_token->length = mk_rep.length;
...
fail:
   if (mk_rep.data)
      krb5_free_data_contents(context, &mk_rep);
