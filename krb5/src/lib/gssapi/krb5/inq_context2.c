...
krb5_gss_inquire_context2(minor_status, context_handle, initiator_name,
			 ...
			 locally_initiated, opened, session_key)
     ...
     gss_buffer_t session_key;
{
   ...
   if (acceptor_name) {
	 ...
	 *minor_status = code;
	 ...
	 if (initiator) {
	    kg_delete_name((gss_name_t) acceptor);
	    krb5_free_principal(context, initiator);
	 }
	 *minor_status = (OM_uint32) G_VALIDATE_FAILED;
	 return(GSS_S_FAILURE);
   ...
   if (session_key) {
	pKeyBuffer = (krb5_octet *)malloc(ctx->subkey->length);
	memcpy(pKeyBuffer, ctx->subkey->contents, ctx->subkey->length);
