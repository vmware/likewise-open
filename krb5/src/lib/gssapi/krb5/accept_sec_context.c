...
static OM_uint32
krb5_gss_wait_for_dce_style(minor_status, context_handle,
     ...
     gss_cred_id_t *delegated_cred_handle;
{
   ...
   krb5_int32 l_seq_number, r_seq_number;

   ap_rep.data = input_token->value;
   ap_rep.length = input_token->length;

   krb5_auth_con_getlocalseqnumber(ctx->k5_context, ctx->auth_context, &l_seq_number);
   krb5_auth_con_getremoteseqnumber(ctx->k5_context, ctx->auth_context, &r_seq_number);

   if ((code = krb5_rd_rep(ctx->k5_context, ctx->auth_context, &ap_rep, &repl))) {
       major_status = GSS_S_FAILURE;
       goto fail;
   }

   /*
    * The client should not send a subkey
    */
   if (repl->subkey) {
   ...
   if (repl->seq_number) {
       if (repl->seq_number != l_seq_number) {
           major_status = GSS_S_UNSEQ_TOKEN;
           goto fail;
       }
   }

   krb5_auth_con_setlocalseqnumber(ctx->k5_context, ctx->auth_context, l_seq_number);
   krb5_auth_con_setremoteseqnumber(ctx->k5_context, ctx->auth_context, r_seq_number);
     ...
     gss_cred_id_t *delegated_cred_handle;
{
   ...
   krb5_principal ap_req_srv_princ = NULL;
   ...
   if (*context_handle != GSS_C_NO_CONTEXT) {
       ...
       return krb5_gss_wait_for_dce_style(minor_status,
                                          context_handle,
	...
	*/
       ...
       /* assume a raw AP_REQ in GSS_C_DCE_STYLE mode */
   ...
   ctx->gss_flags = (GSS_C_TRANS_FLAG |
                    ((gss_flags) & (GSS_C_INTEG_FLAG | GSS_C_CONF_FLAG |
                            ...
                            GSS_C_SEQUENCE_FLAG | GSS_C_DELEG_FLAG | GSS_C_DCE_STYLE)));
   ctx->seed_init = 0;
   ...
   if (ctx->gss_flags & GSS_C_MUTUAL_FLAG) {
       ...
       if (ctx->gss_flags & GSS_C_DCE_STYLE) {
           /* in DCE_STYLE mode no token header is send */
 ...
 fail:
   ...
   *minor_status = code;
   ...
   if (decode_req_message) {
       ...
       if (request->ticket) {
           ap_req_srv_princ = request->ticket->server;
           request->ticket->server = NULL;
       }
	...
	*/
       ...
       if (!cred->princ) {
           krb_error_data.server = ap_req_srv_princ;
       }
...
free:
   if (ap_req_srv_princ) {
	   krb5_free_principal(context, ap_req_srv_princ);
   }
   if (!verifier_cred_handle && cred_handle) {
	   OM_uint32 minor_status2;
	   krb5_gss_release_cred(&minor_status2, &cred_handle);
   }
   krb5_free_context(context);
   return (major_status);
}
