    ...
    int toktype;
{
	...
	*minor_status = code;
	...
	if ((sealalg == 0xffff) && ctx->big_endian) {
		...
		pad_len = plain[tmsglen-1];
            }

	    conflen = kg_confounder_size(context, ctx->enc);
	    token.length = tmsglen - conflen - pad_len;
	}
    ...
    int toktype;
{
    ...
    int err;
    int toktype2;
    unsigned int tok_hdrsize;
    unsigned int extra_bodysize;
...
#define GSS_ARCFOUR_WRAP_TOKEN_DCE_DER_HEADER_SIZE 13
        tok_hdrsize = GSS_ARCFOUR_WRAP_TOKEN_DCE_DER_HEADER_SIZE +
		      GSS_ARCFOUR_WRAP_TOKEN_SIZE;
	if (input_token_buffer->length < tok_hdrsize) {
	    *minor_status = G_BAD_TOK_HEADER;
	    return GSS_S_DEFECTIVE_TOKEN;
	}
	extra_bodysize = input_token_buffer->length - tok_hdrsize;
    } else {
        tok_hdrsize = input_token_buffer->length;
	extra_bodysize = 0;
    }
