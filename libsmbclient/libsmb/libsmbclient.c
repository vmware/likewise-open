                          ...
                          SMBCSRV * srv)
{
	...
	for (file = context->internal->_files; file; file=file->next) {
		if (file->srv == srv) {
			/* Still used */
			DEBUG(3, ("smbc_remove_unused_server: "
                                  "%p still used by %p.\n", 
				  srv, file));
			return 1;
		}
...
#if 0 /* Disabled to deal with Windows 2008 and npcmuxer */
        /*
...
static SMBCFILE *
smbc_nt_creat_ctx(SMBCCTX *context,
                  const char *fname,
...
smbc_new_context(void)
{
	...
	context->options.one_share_per_server      = False;/* backward compat */
        ...
        context->nt_creat                          = smbc_nt_creat_ctx;
