 ...
 int vasprintf(char **ptr, const char *format, va_list ap)
{
	...
	VA_COPY(ap2, ap);
	ret = vsnprintf(dummy, sizeof(dummy) / sizeof(*dummy), format, ap2);
	va_end(ap2);
        
        /* On some platforms, vsnprintf returns -1 on truncation instead
           of the number of bytes that would have been printed */
	if (ret == -1)
        {
            size_t size = 16;
            char* newbuf;
            ...
            do
            {
                ...
                *ptr = newbuf;
                ...
                VA_COPY(ap2, ap);
                ret = vsnprintf(*ptr, size, format, ap2);
                va_end(ap2);
