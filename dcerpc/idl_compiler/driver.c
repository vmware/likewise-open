...

{
    ...
    *sstub_fid  = NULL;
    ...
    if (cmd_opt[opt_header])
    {
        ...
        {
            CALL(FILE_create((char *)cmd_val[opt_header], h_fid), status);
        ...
        &&  cmd_opt[opt_emit_cstub])
    {
        ...
        if (!cmd_opt[opt_confirm])
        {
            CALL(FILE_create((char *)cmd_val[opt_cstub], cstub_fid), status);
        ...
        &&  cmd_opt[opt_emit_sstub])
    {
        ...
        if (!cmd_opt[opt_confirm])
        {
            CALL(FILE_create((char *)cmd_val[opt_sstub], sstub_fid), status);
...
#endif
  {
            ...
            ||  int_p->ool_types != NULL))
    {
        ...
        if (!cmd_opt[opt_confirm])
        {
            CALL(FILE_create((char *)cmd_val[opt_caux], caux_fid), status);
            ...
            ||  int_p->pipe_types != NULL))
    {
        ...
        if (!cmd_opt[opt_confirm])
        {
            CALL(FILE_create((char *)cmd_val[opt_saux], saux_fid), status);
...

{
    ...
    if (setjmp(nidl_termination_jmp_buf) == 0)
    {
         ...
         */
        CALL(CMD_parse_args(argc-1, &argv[1], &cmd_opt, &cmd_val, &idl_sid), status);
         ...
         */
        if (!cmd_opt[opt_confirm])
            CALL(open_fe_files(cmd_opt, cmd_val, &lis_fid), status);

        /* Frontend processing. */
        CALL(FE_main(cmd_opt, cmd_val, idl_sid, &int_p), status);
                ...
                &saux_fid, &cstub_fid, &sstub_fid, int_p))
        {
            ...
            CALL(BE_main(cmd_opt, cmd_val, h_fid, caux_fid, saux_fid,
                         cstub_fid, sstub_fid, int_p), status);
    ...
    {
#if defined(UNIX) || defined(VMS) || defined(_WIN32)
